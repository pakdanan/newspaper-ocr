...
def update_ocr_to_mysql(ocr_base_dir):
    ...
    try:
        ...
        for idx, txt_path in enumerate(txt_files, 1):
            ...
            pdf_file_name = txt_path.stem + ".pdf"
            ...
            print(f"Menyimpan hasil OCR di DB berdasarkan {pdf_file_name}")
            ...
            update_query = "UPDATE pdf_transcriptions t JOIN pdf_article_files p ON p.oldTarkID = t.oldTarkID SET t.OCRText = %s WHERE p.Name LIKE %s"
            
            # Eksekusi langsung ke database memanfaatkan class Database Anda
            result = db.execute(update_query, (ocr_content, f"%{pdf_file_name}"))

            if result:
                print(f"   ✅ Sukses mengeksekusi perintah UPDATE untuk path: '{pdf_file_name}'")
                success_count += 1
            else:
...
def main():
    ...
    parser.add_argument("--ocr-dir", required=True, help="contoh output\\ocr\\1966"  )    
